...
public static void DelegateOnMessageEvent(int instanceId, IntPtr msgPtr, int msgSize)
{
    ...
    if (instances.TryGetValue(instanceId, out instanceRef))
    {
        try 
        {
            if (msgSize <= 0)
            {
                Debug.LogWarning("Received empty message in DelegateOnMessageEvent");
                return;
            }
            ...
            // Попробуем интерпретировать как строку UTF8
            try 
            {
                string messageStr = Encoding.UTF8.GetString(msg);
                //Debug.Log($"Received message as string: {messageStr}");
                
                // Если это JSON, передаем как есть
                if (messageStr.StartsWith("{") || messageStr.StartsWith("["))
                {
                    // Конвертируем обратно в байты для единообразия
                    byte[] jsonBytes = Encoding.UTF8.GetBytes(messageStr);
                    instanceRef.DelegateOnMessageEvent(jsonBytes);
                    return;
                }
            }
            catch
            {
                // Не UTF8 строка, передаем как бинарные данные
                //Debug.Log($"Received binary message of size: {msgSize}");
                instanceRef.DelegateOnMessageEvent(msg);
            }
        }
        catch (Exception e)
        {
            Debug.LogError($"Error in DelegateOnMessageEvent: {e}\n{e.StackTrace}");
        }
    }
}

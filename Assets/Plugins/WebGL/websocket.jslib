    ...
    WebSocketConnect: function(instanceId) {
        ...
        instance.ws.onmessage = function(ev) {
            ...
            if (webSocketState.onMessage === null) {
                console.warn("onMessage callback is null");
                return;
            }

            try {
                if (typeof ev.data === 'string') {
                    // Преобразуем строку в UTF8 байты
                    var msgBytes = lengthBytesUTF8(ev.data);
                    var buffer = _malloc(msgBytes + 1); // +1 для нуль-терминатора

                    // Записываем строку в буфер
                    stringToUTF8(ev.data, buffer, msgBytes + 1);

                    Module.dynCall_viii(webSocketState.onMessage, instanceId, buffer, msgBytes);
                ...
                console.error("[JSLIB] Error processing message:", e);
            }
        };
